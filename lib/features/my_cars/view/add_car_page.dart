...
class _AddCarPageState extends State<AddCarPage> {
  ...
  Future<void> _submitForm() async {
    ...
    if (allValid) {
      await controller.addCar(isRefresh: false, context: context);
      if (context.mounted) {
        Get.back();
      }
    } else {
        ...
        children: [
          ...
          Expanded(
            ...
            child: GlobalElevatedButton(
              text: _currentStep == _totalSteps - 1 ? tr('add car') : tr('next'),
        ...
        GlobalDropdown<CarBodyType>(
          ...
          items: controller.allCarBodyTypes
              .map((bodyType) => DropdownItem(
                    value: bodyType,
                    label: bodyType.name ?? 'Unknown Body Type',
                  ))
              .toList(),
        ...
        GlobalTextFormField(
          ...
          validator: (value) {
            ...
            if (value.trim().length < 3) {
              return tr('title must be at least 3 characters');
            }
            return null;
          },
        ),
  ...
  Widget _buildColorSelectionSection(String type, String label) {
    return Column(
      ...
      children: [
        Text(
          '${tr('available')} ${label.capitalizeFirst} ${tr('colors')}',
          style: const TextStyle(
            fontSize: 16,
            fontWeight: FontWeight.w500,
          ),
  ...
  Widget _buildColorImagesSection(String type, String label) {
    ...
    return Column(
      ...
      children: [
        Text(
          '${label.capitalizeFirst} ${tr('color images')}',
          style: const TextStyle(
            fontSize: 16,
            fontWeight: FontWeight.w500,
          ),
                    ...
                    return Stack(
                      children: [
                        ...
                        Positioned(
                          ...
                          right: 2,
                          child: GestureDetector(
                            onTap: () => controller.removeColorImage(color, index),
                            child: Container(
